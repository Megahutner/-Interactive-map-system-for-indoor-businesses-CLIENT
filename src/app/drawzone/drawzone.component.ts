...
@HostListener('document:keydown', ['$event'])
  keyPress(e) { // key input when using interactive map
    var overlap=false
...
zoneEnum: any = [];
  ...
  // Interactive map related functionalities
...
changeXY(e:CdkDragEnd,item:any){ // cdkDragEnded event
    ...
    if(object.name==item.name){
     ...
     //   if(!notOverlapped ){
      ...
      if(object.distance.x < 0){ // left boundary
        object.distance.x = objectFreePositionX;
        object.dragPosition.x=objectFreePositionX;
      }
      if(object.distance.y < 0){ // top boundary
        object.distance.y = objectFreePositionY;
        object.dragPosition.y = objectFreePositionY;
      }
  ...
  // End of  Interactive map related functionalities
...
//////// end of pathfinding functionalties

// CRUD object functionalities
getZoneDetails(id){
...
openCreate(){
  ...
  this.editMode = false;
  this.currentRow.name = "Object";
  this.currentRow.width = 100;
...
// saveData(){
    ...
    // }  
// End of CRUD object functionalities 

// Image/Background functionalities //
ChangeBG() {
...
// End of Image/Background functionalities //

// Category functionalities //
public getEnumZones(id){
  this.apiService.GetZoneEnum(id).then((res) => {
    this.zoneEnum = res?.data;
  })
  .catch((err) => {
    this.apiService.error(err);
  });
...
rotate(item){   // change current highlighted block 's rotate value
  ...
  focus(item){      // highlight a block upon mouseover
    ...
    mouseMove(e){
      if(this.mouseDownStatus && this.currentBlockID == null){
        var containerX = document.getElementById('container').getBoundingClientRect().x ;
        var containerY = document.getElementById('container').getBoundingClientRect().y ;
